...
(function (EVA) {
  ...
  function renderSidebar() {
    ...
    var statusTitle = tvStats.live
      ? EVA.services.tv.currentLabel()
      : "Display stopped";
    var statusMeta = tvStats.live
      ? tvStats.slides +
        " " +
        U.pluralize(tvStats.slides, "slide") +
        " · " +
        U.duration(tvStats.loopSeconds) +
        " loop"
      : "Nothing is playing";
    ...
    return (
      ...
      '<div class="brand">' +
      '<span class="brand__mark"><img src="assets/askeva%20logo.jfif" alt="Ask EVA logo"></span>' +
      '<div class="brand__text">' +
      '<div class="brand__name">' +
      U.esc(name.replace(/EVA$/i, "")) +
      (/eva$/i.test(name) ? "<em>EVA</em>" : "") +
      "</div>" +
      '<div class="brand__sub">' +
      U.esc(settings.tagline || "Office TV") +
      "</div>" +
      "</div>" +
      "</div>" +
      '<nav class="nav">' +
      ...
      '<span class="tv-status__label">' +
      statusLabel +
      "</span>" +
      "</div>" +
      '<div class="tv-status__title">' +
      U.esc(statusTitle) +
      "</div>" +
      '<div class="tv-status__meta">' +
      icon("layers", { size: 12 }) +
      "<span>" +
      U.esc(statusMeta) +
      "</span></div>" +
  ...
  function renderHeader() {
    ...
    var pageTitle = page.getTitle
      ? page.getTitle(app.route.params)
      : page.title;
    ...
    return (
      ...
      U.attr(EVA.services.settings.tvUrl()) +
      '" target="_blank" rel="noopener">' +
      icon("external-link", { size: 15 }) +
      "Open TV" +
      "</a>" +
      ...
      icon("bell") +
      (pending.changed ? '<i class="icon-btn__badge"></i>' : "") +
      "</button>" +
      ...
      icon("tv") +
      "Open TV display</a>" +
      '<button class="menu__item" type="button" data-app-action="preview">' +
      icon("eye") +
      "Preview TV</button>" +
      '<div class="menu__sep"></div>' +
      '<button class="menu__item" type="button" data-app-action="reseed">' +
      icon("refresh") +
      "Reset demo data</button>" +
      '<button class="menu__item is-danger" type="button" data-app-action="signout">' +
      icon("log-out") +
      "Sign out</button>" +
      "</div>" +
      "</div>" +
  ...
  function renderShell() {
    ...
    root.innerHTML =
      ...
      '<div class="main">' +
      renderHeader() +
      '<main class="page" id="pageRoot"></main></div>';
    bindShell();
  }
  ...
  function renderPage(keepScroll) {
    ...
    document.title =
      (page.getTitle ? page.getTitle(app.route.params) : page.title) +
      " · AskEVA Office TV";
  ...
  function bindShell() {
    ...
    if (search) {
      search.onkeydown = function (e) {
        if (e.key === "Enter" && search.value.trim()) {
          ...
          search.value = "";
          if (EVA.pages.employees.setSearch)
            EVA.pages.employees.setSearch(term);
          app.go("employees");
        }
      };
    }
    ...
    Array.prototype.forEach.call(
      ...
      function (el) {
        el.onclick = function () {
          appAction(el.dataset.appAction);
        };
      },
    );
  }
  ...
  function appAction(action) {
    ...
    if (action === "push") {
      ...
      ui.toast.info(
        "Single-admin demo",
        "Sign-in is not part of this prototype.",
      );
    }
  }
  ...
  function showPending() {
    ...
    var body = p.changed
      ? '<p class="confirm__text" style="text-align:left;margin-bottom:14px">You have content that is not on the office TV yet.</p>' +
        '<div class="kv"><span class="kv__k">Slides ready</span><span class="kv__v">' +
        p.count +
        "</span></div>" +
        '<div class="kv"><span class="kv__k">New since last push</span><span class="kv__v">' +
        p.added +
        "</span></div>" +
        '<div class="kv"><span class="kv__k">Removed</span><span class="kv__v">' +
        p.removed +
        "</span></div>" +
        '<div class="kv"><span class="kv__k">Currently live</span><span class="kv__v">' +
        live.slides +
        "</span></div>"
      : ui.empty({
          icon: "check-circle",
    ...
    ui.modal({
      ...
      sub: live.publishedAt
        ? "Last push " + U.timeAgo(live.publishedAt)
        : "Never published",
      icon: "bell",
      size: "sm",
      body: body,
      ...
      onMount: function (ctrl) {
        ...
        if (b)
          b.onclick = function () {
            ctrl.close();
            EVA.publish.push({}).then(function (ok) {
              if (ok) app.refresh();
            });
          };
      },
  ...
  app.boot = function () {
    ...
    EVA.store.subscribe(function () {
      if (app.booted) debounced();
    });
    ...
    if (due.length) {
      due.forEach(function (a) {
        EVA.services.announcements.publish(a.id);
      });
      ui.toast.info(
        due.length +
          " scheduled " +
          U.pluralize(due.length, "announcement") +
          " went live",
