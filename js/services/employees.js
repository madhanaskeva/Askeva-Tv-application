...
(function (EVA) {
  ...
  var U = EVA.utils;
  var COLL = "employees";
  var ROLE_COLL = "employeeRoles";

  store.defaults[COLL] = EVA.seed.employees;
  store.defaults[ROLE_COLL] = function () {
    return [];
  };
  ...
  var service = {
    ...
    query: function (q) {
      ...
      if (term) {
        rows = rows.filter(function (e) {
          return (
            [e.name, e.role, e.department, e.employeeId, e.email]
              .join(" ")
              .toLowerCase()
              .indexOf(term) > -1
          );
      ...
      if (q.department && q.department !== "all") {
        rows = rows.filter(function (e) {
          return e.department === q.department;
        });
      }
      if (q.role && q.role !== "all") {
        rows = rows.filter(function (e) {
          return e.role === q.role;
        });
      }
      if (q.status && q.status !== "all") {
        rows = rows.filter(function (e) {
          return e.status === q.status;
        });
      }
    ...
    departments: function () {
      var used = store.list(COLL).map(function (e) {
        return e.department;
      });
    ...
    roles: function () {
      ...
      var extras = store.list(ROLE_COLL).map(function (role) {
        return role.name || role;
      });
      return U.unique(used.concat(extras)).filter(Boolean).sort();
    },

    addRole: function (name) {
      var trimmed = String(name || "").trim();
      if (!trimmed) return null;
      var list = store.list(ROLE_COLL);
      var exists = list.some(function (role) {
        return (
          String(role.name || role).toLowerCase() === trimmed.toLowerCase()
        );
      });
      if (exists) return null;
      var next = {
        id: EVA.utils.uid("role"),
        name: trimmed,
        createdAt: new Date().toISOString(),
      };
      list.push(next);
      store.writeDoc(ROLE_COLL, list, { action: "role-add" });
      return next;
    },
    ...
    validate: function (data, id) {
      ...
      if (U.required(d.employeeId))
        ...
        if (clash)
          errors.employeeId = "That ID is already used by " + clash.name;
      }

      if (U.required(d.role)) errors.role = "Role is required";
      if (U.required(d.category)) errors.category = "Pick an employee category";
    ...
    nextEmployeeId: function () {
      var nums = store
        .list(COLL)
        .map(function (e) {
          return parseInt(String(e.employeeId).replace(/\D/g, ""), 10);
        })
        .filter(function (n) {
          return !isNaN(n);
        });
    ...
    create: function (data) {
      ...
      EVA.services.activity.log(
        "employee",
        "Employee <strong>" +
          U.esc(rec.name) +
          "</strong> added to the directory",
      );
      return rec;
    },
    ...
    stats: function () {
      ...
      return {
        ...
        onLeave: rows.filter(function (e) {
          return e.status === "on-leave";
        }).length,
        inactive: rows.filter(function (e) {
          return e.status === "inactive";
        }).length,
        departments: U.unique(
          rows.map(function (e) {
            return e.department;
          }),
